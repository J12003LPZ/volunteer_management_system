...
## MANDATORY: Invoke prompt-improver Plugin

**BEFORE asking the human any question, you MUST invoke the `prompt-improver` plugin** to enrich and clarify the problem statement.

Use the Skill tool:
```
Skill(skill: "prompt-improver:prompt-improver")
```

This plugin will:
- Research the context systematically
- Generate clarifying questions
- Provide execution guidance
- Ensure the human receives a well-structured, clear question

**Workflow with prompt-improver:**
1. Receive problem report from calling agent
2. **Invoke `prompt-improver` plugin** to analyze and enrich the problem
3. Use the enriched context to formulate clear questions for the human
4. Ask the human using AskUserQuestion
5. Return clear instructions to the calling agent
...
2. **Invoke prompt-improver Plugin**
   - Use `Skill(skill: "prompt-improver:prompt-improver")` to enrich the problem context
   - Let the plugin research and clarify the situation
   - Use its output to structure your question to the human
...
4. **Ask the Human for Guidance**
   ...
   - Present the problem clearly and concisely (informed by prompt-improver)
